...
%><cache:cache key="<%=SITE_URL+"story/"+currentPageNumber+"/"+id%>" scope="application " time="1" refresh="t" ><%
ArrayList storyData = (ArrayList) StoryHelper.storyDetail(id, 1);
if (storyData != null && storyData.size() > 0) {
	//System.out.println("BTWAP story.jsp:: ========>> "+storyData.size());
	StoryDTO aDTO = null;
...
.storytext p{font-size:16px; line-height:21px;}
.storycontent > img{width:100%;}
@media only screen and (min-device-width : 300px) and (max-device-width : 767px) { p iframe{height:250px !important;}
.tbl-forkorts-article.tbl-forkorts-article-active{ max-height:400px !important;}
 }
 
 .tbl-forkorts-article.tbl-forkorts-article-active{ max-height:400px;}
...
<script type="text/javascript" language="javascript" src="http://media2.intoday.in/businesstoday/1.1.6/resources/js/ajaxinclude.js"></script> 
</head><body> 
...
<% if(!StringUtil.isBlank(aDTO.getLargeKickerImage())) { 	%>
		<tr class="storycontent"><td align="center"><img rel="prefetch" src="<%= STORY_IMAGE_BASEPATH + aDTO.getLargeKickerImage()%>" width="320" height="170" border="0" align="middle" alt="<%= aDTO.getLargeKickerImageAltText()%>" title="<%=aDTO.getLargeKickerImageAltText()%>" />
...
<div style="width:100%; text-align:center;">
 <!-- Ad Slot 1 tag: --> 
 <!-- begin ZEDO for channel:- BT ROS Middle 1, Publisher: Bussiness Today WAP, Ad Dimension: Medium Rectangle-300x250 --> 
...
if(typeof zmt_mtag !='undefined' && typeof zmt_mtag.zmt_render_placement !='undefined')
{
 ...
 <!-- end ZEDO for channel: - BT ROS Middle 1, Publisher: Bussiness Today WAP, Ad Dimension: Medium Rectangle-300x250 --> 
</div>
</td>
</tr>
 <div style="height:5px;"></div> 
...
<jsp:include page="bottomchartbeat.jsp" >
	<jsp:param value="<%=aDTO.getSectionTitle() %>" name="chartBeatSecName"/>
	<jsp:param value="<%=aDTO.getByline() %>" name="chartBeatAuthName"/>
	<jsp:param value="<%= StringUtil.replaceSingleQuote(StringUtil.isEmpty(aDTO.getTitleAlias())) %>" name="chartBeatTitleName"/>
</jsp:include>
</body>
</html>
<%	}
