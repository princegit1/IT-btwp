...
%>
<cache:cache key="<%=SITE_URL+"subcategory/"+currentPageNo+"/"+sectionId%>" scope="application " time="300" refresh="t" >
...
ArrayList sAL =null;				
sAL = CommonFunctions.latestStoriesTemplate(sectionId, ctrToFetch, "0", 3);
...
<%@ include file="topnav.jsp"%><%sectionNameforAddDisplay=sDTO.getSubCategoryTitle();%>
...
<tr><td class="breadcrumb">
	<a href="<%=SITE_URL%>">Home</a> &raquo; <a href="<%=CommonFunctions.sectionURL(sDTO.getSectionSefTitle(),1, sDTO.getSectionId())%>"><%=sDTO.getSectionTitle()%></a> &raquo;  <a href="<%=CommonFunctions.categoryURL(sDTO.getCategorySefTitle(),1, sDTO.getCategoryId())%>"><%=sDTO.getCategoryTitle()%></a> &raquo; <%=sDTO.getSubCategoryTitle()%>

</td></tr>
<tr><td height="7"></td></tr><tr><td bgcolor="#2E3094" class="subheading"><%=sDTO.getSubCategoryTitle()%> </td></tr>
<%}%>
...
<%if(!sDTO.getTitle().contains("http://")){%>
	...
	<%if(sDTO.getImage().trim().length() > 0) {%>
		<a href="<%=CommonFunctions.storyURL(sDTO.getSefTitle(),1, sDTO.getId())%>"><img src="http://media2.intoday.in/btmt/images/stories/<%=sDTO.getImage().trim()%>" width="80" height="80" align="left" style="margin:0 8px 0 2px; border:0;"/></a>
	<% } else {%>
