...
if(request.getParameter("id")!=null)
	videoId =  Integer.parseInt((String)request.getParameter("id"));
	...
	<%
	  ...
	  $(document).ready(function() {
        ...
        },
		{
          type: 'download',
          config: {
           'file': '<%if(ThreeGPvideoPathFinalnew[0]!=null &&ThreeGPvideoPathFinalnew[0].length()>0){ThreeGPvideoPathFinalnew[0].replaceAll("\"","");} %>',
           'provider': 'video'
          }
        }
    ],   
                     ...
                     "levels": "none" },
							  ...
							  'companions': {
                                ...
                                'schedule': [
                                    ...
                                    <%}else {%>
									<% if(splittedArray.length==1){%>
 'tag': "https://pubads.g.doubleclick.net/gampad/ads?sz=640x480|400x300&iu=/1007232/m.businesstoday.in_VOD_Pre_roll&impl=s&gdfp_req=1&env=vp&output=xml_vast2&unviewed_position_start=1&url=[referrer_url]&description_url=[description_url]&correlator=[timestamp]",
										 <%} else{%>
'tag': "https://pubads.g.doubleclick.net/gampad/ads?sz=640x480|400x300&iu=/1007232/m.businesstoday.in_VOD_Pre_roll&impl=s&gdfp_req=1&env=vp&output=xml_vast2&unviewed_position_start=1&url=[referrer_url]&description_url=[description_url]&correlator=[timestamp]",
                                    <%}}%>
                                    'duration': 25
