...
%>
<cache:cache key="<%=SITE_URL+"latest_stories_chunk.jsp?categoryId="+categoryId+"&contentCount="+contentCount%>" scope="application" time="119" refresh="t" > 
...
if (lsAL != null && lsAL.size() > 0) {
	for (int ctr = 0; ctr < lsAL.size(); ctr++) {
		...
		if (ctr == 0) {
			...
			if(categoryId.equals("178"))
				headerURL=SITE_BASEPATH+"budget/";
				
			if(categoryId.equals("186"))
				headerURL=SITE_BASEPATH+"budget/2015/";
				...
				catTitle="Opinion";
				}else if(cDTO.getCurrentCategoryTitle().equals("Year Roundup: 2014") || cDTO.getCurrentCategoryTitle().equals("YEAR ROUNDUP: 2014")){
				catTitle="YEAR 2014: RECAP";
				}else{
